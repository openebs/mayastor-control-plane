...
pub trait JsonGrpc: Clone {
    async fn put_node_jsongrpc(
        &self,
        node: &str,
        method: &str,
        body: serde_json::Value,
    ) -> Result<serde_json::Value, Error<crate::models::RestJsonError>>;
}
...
impl JsonGrpc for JsonGrpcClient {
    async fn put_node_jsongrpc(
        ...
        let local_var_uri_str = format!(
            ...
            method = crate::apis::client::urlencode(method)
        );
        ...
        let mut local_var_resp = if configuration.trace_requests {
            local_var_req_builder.send_json(&body).await
        } else {
            local_var_req_builder.trace_request().send_json(&body).await
        }?;

        let local_var_status = local_var_resp.status();

        if local_var_status.is_success() {
            let local_var_content = local_var_resp.json::<serde_json::Value>().await?;
