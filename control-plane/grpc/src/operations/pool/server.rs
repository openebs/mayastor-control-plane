use crate::{
    ...
    pool::{
        ...
        unlabel_pool_reply, AbortDrainPoolReply, AbortDrainPoolRequest, ClearErrorsReply,
...
impl PoolGrpc for PoolServer {
    ...
    async fn abort_drain_pool(
        &self,
        request: Request<AbortDrainPoolRequest>,
    ) -> Result<tonic::Response<AbortDrainPoolReply>, tonic::Status> {
        let request = request.into_inner();
        match self.service.abort_drain(&request.try_into()?).await {
            Ok(pool) => Ok(Response::new(AbortDrainPoolReply {
                reply: Some(abort_drain_pool_reply::Reply::Pool(pool.into())),
            })),
            Err(err) => Ok(Response::new(AbortDrainPoolReply {
                reply: Some(abort_drain_pool_reply::Reply::Error(err.into())),
            })),
        }
    }
}
