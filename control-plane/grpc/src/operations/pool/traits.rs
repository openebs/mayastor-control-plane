...
/// Pool drain information.
#[derive(Debug, Clone)]
pub struct AbortDrainPoolRequest {
    /// Node ID of where the pool resides on.
    /// This is optional and may be used for stricter checks.
    pub node_id: Option<NodeId>,
...
impl TryFrom<pool::AbortDrainPoolRequest> for AbortDrainPoolRequest {
    type Error = ReplyError;

    fn try_from(value: pool::AbortDrainPoolRequest) -> Result<Self, Self::Error> {
        Ok(Self {
            node_id: value.node_id.map(Into::into),
            pool_id: value.pool_id.into(),
        })
...
impl From<AbortDrainPoolRequest> for pool::AbortDrainPoolRequest {
    fn from(value: AbortDrainPoolRequest) -> Self {
        Self {
            node_id: value.node_id.map(Into::into),
            pool_id: value.pool_id.into(),
        }
