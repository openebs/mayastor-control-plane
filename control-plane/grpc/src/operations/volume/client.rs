use crate::{
    ...
    volume::{
        ...
        trigger_rebuild_reply, unpublish_volume_reply, volume_grpc_client::VolumeGrpcClient,
        GetSnapshotsRequest, GetVolumesRequest, ProbeRequest,
    },
};
use stor_port::{
...
impl VolumeOperations for VolumeClient {
    ...
    #[tracing::instrument(
        name = "VolumeClient::trigger_rebuild",
        level = "info",
        skip(self),
        err
    )]
    async fn trigger_rebuild(
        ...
        let req = self.request(req, ctx, MessageIdVs::TriggerRebuild);
        let response = self.client().trigger_rebuild(req).await?.into_inner();
        match response.reply {
            Some(trigger_rebuild_reply) => match trigger_rebuild_reply {
                trigger_rebuild_reply::Reply::Volume(volume) => Ok(Volume::try_from(volume)?),
                trigger_rebuild_reply::Reply::Error(err) => Err(err.into()),
            },
            None => Err(ReplyError::invalid_response(ResourceKind::Volume)),
        }
    }
