...
/// promptly when no cap is set; the pair is what attributes the deferral here to
/// the cap rather than to the grace period or a viability check.
#[tokio::test]
async fn offline_rebuild_respects_max_offline_rebuilds() {
    let reconcile = Duration::from_millis(RECONCILE_PERIOD_MS);
    // Scaling up bypasses the grace period, so the reconciler would act within a
    // poll or two. Allow the grace period on top anyway as headroom.
    let observe = Duration::from_secs(GRACE_PERIOD_SECS) + reconcile * 2;
    let cluster = ClusterBuilder::builder()
        .with_rest(true)
        .with_io_engines(2)
        .with_tmpfs_pool(52428800)
    ...
    let volid = VolumeId::new();
    let body = models::CreateVolumeBody::new(VolumePolicy::new(true), 1, 10485760u64, false, false);
    let volume = volume_api.put_volume(&volid, body).await.unwrap();
    let uid = volume.spec.uuid;
    ...
    volume_api
        .put_volume_replica_count(&uid, 2)
        .await
        .expect("Should scale up to 2 replicas");
...
async fn offline_rebuild_can_be_requested_by_operator() {
    ...
    // Nothing should happen on its own: the grace period has barely started.
