...
impl ResourcePublishing for OperationGuardArc<VolumeSpec> {
    ...
    async fn unpublish(
        ...
        let demote = registry.offline_rebuild_enabled()
            && state.target.as_ref().is_some_and(|t| t.rebuilds > 0);

        let op = VolumeOperation::Unpublish(if demote {
            UnpublishOperation::new_demote(host_acls.clone())
        } else {
            UnpublishOperation::new(host_acls.clone())
        });
        ...
        let result = match specs.nexus_opt(volume_target.nexus()).await? {
            ...
            Some(mut nexus) if last_node && demote => {
                // `demote` was derived from `state.target`, so it is present here.
                let target = state.target.as_ref().expect("demote implies a target");
