...
use stor_port::{
    ...
    types::v0::{
        store::{
            ...
            volume::{TargetConfig, VolumeGroupSpec, VolumeOperation, VolumeSpec},
        ...
        transport::{
            ...
            ReplicaOwners, ShareReplica, Volume, VolumeId, VolumeShareProtocol, VolumeState,
            VolumeStatus,
        },
    },
};
...
impl ResourceSpecsLocked {
    ...
    pub(super) fn remove_volume_group(&self, id: &VolumeId, vg_id: &String) {
        let mut specs = self.write();
        if let Some(vg_spec) = specs.volume_groups.get(vg_id) {
            vg_spec.lock().remove(id);
            if vg_spec.lock().is_empty() {
