Feature: Replica Anti-Affinity for Volume Group

  Background:
    Given an io-engine cluster

  # Volume Groups to have volumes with single replicas

  Scenario Outline: creating volume group with 3 volumes with 1 replica each
    Given 5 pools, 2 on node A, 1 on node B, 2 on node C
    When the create volume request with volume group and <replicas> and <thin> executed
  ...
  Scenario Outline: creating volume group with 3 volumes with 1 replica each with limited pools
  ...
  # Volume Groups to have volumes with multiple replicas.

  Scenario Outline: creating volume group with 3 volumes with 3 replica on 9 pools
  ...
  Scenario Outline: creating volume group with 3 volumes with 3 replica on 5 pools
    Given 5 pools, 2 on node A, 1 on node B, 2 on node C
    When the create volume request with volume group and <replicas> and <thin> executed
    Then 3 volumes should be created with 3 replica each
  ...
  Scenario Outline: creating volume group with 3 volumes with 3 replica on 3 pools
