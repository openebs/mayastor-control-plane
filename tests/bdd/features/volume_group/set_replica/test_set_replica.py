"""Anti-Affinity for Volume Group feature tests."""
...
@scenario("feature.feature", "volume group volumes are scaled down")
def test_volume_group_volumes_are_scaled_down():
    """volume group volumes are scaled down."""


@scenario("feature.feature", "volume group volumes are scaled down below 2 replicas")
def test_volume_group_volumes_are_scaled_down_below_2_replicas():
    """volume group volumes are scaled down below 2 replicas."""


@scenario("feature.feature", "volume group volumes are scaled up")
def test_volume_group_volumes_are_scaled_up():
    """volume group volumes are scaled up."""
...
@given(
    parsers.parse(
        "3 volumes belonging to a volume group with {replicas:d} each and {thin}"
    )
)
def volumes_belonging_to_a_volume_group_with_replicas_each_and_thin(replicas, thin):
    """3 volumes belonging to a volume group with <replicas> each and <thin>."""
...
def the_3_volumes_are_scaled_down_to_2():
    ...
    scale_volume_group(2)


@when("the 3 volumes are scaled up by one")
def the_3_volumes_are_scaled_up_by_one():
    """the 3 volumes are scaled up by one."""
    scale_volume_group(3)


@when(
    parsers.parse(
        "the volume belonging to a volume group with one replica and {thin} is created"
    )
)
def the_volume_belonging_to_a_volume_group_with_one_replica_and_thin_is_created(thin):
    """the volume belonging to a volume group with one replica and <thin> is created."""
    try:
        if pytest.vol_id != "":
            ...
            pytest.vol_id = VG_VOLUME_UUID_2
...
@then("each node should have a maximum of 2 replicas from the volumeGroup")
def each_node_should_have_a_maximum_of_2_replicas_from_the_volumegroup():
    """each node should have a maximum of 2 replicas from the volumeGroup."""
...
def scale_volume_group(replicas):
