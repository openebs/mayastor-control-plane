...
@scenario(
    "pool-affinity-spread.feature", "desired number of replicas cannot be created"
)
...
def a_request_for_a_volume_with_its_topology_exclusion_labels_key_with_empty_value_matching_with_any_pools_with_labels(
    create_request,
):
    """a request for a volume with its topology exclusion labels key with empty value matching with any pools with labels."""
...
def a_request_for_a_volume_with_its_topology_exclusion_labels_key_matching_with_any_pools_with_labels(
    create_request,
):
    """a request for a volume with its topology exclusion labels key matching with any pools with labels."""
    request = CreateVolumeBody(
...
def a_request_for_a_volume_with_its_topology_inclusion_labels_not_matching_with_any_pools_with_labels(
    create_request,
):
    """a request for a volume with its topology inclusion labels not matching with any pools with labels."""
    request = CreateVolumeBody(
...
def a_request_for_a_volume_with_its_topology_inclusion_labels_matching_certain_pools_with_same_labels(
    create_request,
):
    """a request for a volume with its topology inclusion labels matching certain pools with same labels."""
    request = CreateVolumeBody(
...
def a_request_for_a_volume_with_its_topology_inclusion_labels_key_with_empty_value_matching_certain_pools_with_same_labels(
    create_request,
):
    """a request for a volume with its topology inclusion labels key with empty value matching certain pools with same labels."""
    request = CreateVolumeBody(
...
def the_volume_replicas_has_topology_exclusion_labels_key_matching_with_any_pools_with_labels_key_and_not_value(
    create_request,
):
    """the volume replicas has topology exclusion labels key matching with any pools with labels key and not value."""
    num_volume_replicas = create_request[CREATE_REQUEST_KEY]["replicas"]
...
def the_volume_replicas_has_topology_inclusion_labels_not_matching_with_any_pools_with_labels(
    create_request,
):
    """the volume replicas has topology inclusion labels not matching with any pools with labels."""
    num_volume_replicas = create_request[CREATE_REQUEST_KEY]["replicas"]
...
def the_volume_replicas_has_topology_inclusion_labels_key_with_empty_value_matching_to_the_labels_as_that_of_certain_pools(
    create_request,
):
    """the volume replicas has topology inclusion labels key with empty value matching to the labels as that of certain pools."""
    num_volume_replicas = create_request[CREATE_REQUEST_KEY]["replicas"]
...
def the_volume_replicas_has_topology_inclusion_labels_matching_to_the_labels_as_that_of_certain_pools(
    create_request,
):
    """the volume replicas has topology inclusion labels matching to the labels as that of certain pools."""
    num_volume_replicas = create_request[CREATE_REQUEST_KEY]["replicas"]
...
def volume_creation_should_succeed_with_a_returned_volume_object_with_exclusion_topology_key_same_as_pool_key_and_different_value(
    create_request,
):
    """volume creation should succeed with a returned volume object with exclusion topology key same as pool key and different value."""
    expected_spec = VolumeSpec(
...
def volume_creation_should_succeed_with_a_returned_volume_object_with_topology_label_key_with_empty_value(
    create_request,
):
    """volume creation should succeed with a returned volume object with topology label key with empty value."""
    expected_spec = VolumeSpec(
...
def volume_creation_should_succeed_with_a_returned_volume_object_with_exclusion_topology_key_different_as_pool_key(
    create_request,
):
    """volume creation should succeed with a returned volume object with exclusion topology key different as pool key."""
    expected_spec = VolumeSpec(
...
def no_of_suitable_pools(
    volume_pool_topology_inclusion_labels, volume_pool_topology_exclusion_labels
):
    pool_with_labels = {
    ...
    for pool_id, labels in pool_with_labels.items():
        if does_pool_qualify_inclusion_labels(
            volume_pool_topology_inclusion_labels, labels
        ) and does_pool_qualify_exclusion_labels(
            volume_pool_topology_exclusion_labels, labels
        ):
...
def does_pool_qualify_exclusion_labels(
    volume_pool_topology_exclusion_labels, pool_labels
):
